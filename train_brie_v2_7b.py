#!/usr/bin/env python3
"""Full SFT training script for Brie v2 (7B) - TRL 0.23 - 1 epoch to reduce overfitting"""
...
# LoRA config - slightly higher dropout for 7B to prevent overfitting
...
# Training config - 1 EPOCH for 7B model to prevent overfitting on smaller dataset
sft_config = SFTConfig(
    output_dir='runs/brie-v2-7b',
    num_train_epochs=1,  # Only 1 epoch for 7B!
    ...
    warmup_steps=10,  # Reduced warmup for 1 epoch
    logging_steps=10,
    eval_strategy='steps',
    eval_steps=50,
    save_steps=100,
...
print("\nSaving final model...")
trainer.save_model()
print(f"Model saved to: runs/brie-v2-7b/")
